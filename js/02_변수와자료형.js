...
function scopeTest2(){
  ...
  if(1 == 1){ // 1과 1이 같다면 if {} 수행
    ...
    // 넓은 범위(함수)에서 선언 된 변수는 
    // 좁은범위(함수 내 if)에서 사용 가능!!

    console.log("if 내부에서 fn1: ", fn1); // 출력 0
    
    
    console.log("if 내부에서 local1 : ", local1); // 출력 0
    console.log("global1 : ", global1); // 출력 0
  ...
  console.log("if실행 후 fn2: ", fn2); // 변경된 fn2
...
function typeCheck(){
  ...
  /* undifined : 정의되지 않은 변수 / 값 대입이 안되어있는 상태 또는 존재 X 변수 */
