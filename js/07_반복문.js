...
function check4() {
  ...
  for (let num = 1; num <= 5; num++) {     // num / result / result + num = num
...
function check13() {
  ...
  let str = ''; // 출력된 문자열을 저장 할 변수


  for (let num = 1; num <= 20; num++) { // 1부터 20까지 1씩 증가하는 반복문 작성
...
function check21() {
  ...
  let count = 0; // 배수 카운트는 for문 밖으로
  for (let num = 1; num <= 100; num++) {

    if (num % value === 0) { // 입력받은 값
      count++;
    }
  }
  alert(`count : ${count}`);
...
function check24() {
  ...
  let gimbapCount    = 0;           // 김밥 주문개수 카운트 하는 변수
  let ramenCount     = 0;           // 라면 
  let donkkaseuCount = 0;           // 돈까스

  // prompt에 입력된 값을 저장할 변수
  let input; // 선언만 한 경우 indifined 

  // input이 undefined 이기 때문에
  // while문 첫 반복은 무조건 수행됨!!
  while (input !== null) { // input null이 아니면 반복

    //  prompt -> 확인 : 작성한 값 반환
    //  prompt -> 취소 : null
    //         -> 취소 선택 시 while 문 다음 반복수행 X
    input = prompt("메뉴 번호 입력");

    // input 값에 따라서 안맞는 case 수행
    switch (input) {
  ...
  let html = ''; // 화면에 출력된 HTML 코드를 저장할 변수

  if (gimbapCount > 0) {
    html += `<li>김밥(${gimbapCount}개) : ${gimbapCount * gimbap}원</li>`
  }
  if (ramenCount > 0) {
    html += `<li>김밥(${ramenCount}개) : ${ramenCount * ramen}원</li>`
  }
  if (donkkaseuCount > 0) {
    html += `<li>김밥(${donkkaseuCount}개) : ${donkkaseuCount * donkkaseu}원</li>`
  }
  // 주문한 메뉴 합계 저장
