...
document.querySelector("#btn3").addEventListener("click", function () {
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    numberArr[i] = num; // 입력된 값이 numberArr에 저장
  }
...
function pow(num, x) {
  ...
  let result = 1; // 곱할 때 영향을 주지않는 숫자 1로 초기화
...
// 익명함수를 이벤트 핸들러로 많이 사용 하는 이유
// -> 해당 이벤트 전용 기능인 경우가 많아서!!
//    (다른 곳에선 재사용을 못하는 경우가 많다)
...
// 이름있는 함수 선언/정의 -> 여러 곳에서 재사용이 가능
function testFn6() {
  console.log("이름 있는 함수 실행");
}

// 함수명만 작성 -> 함수 코드 그대로 반환
