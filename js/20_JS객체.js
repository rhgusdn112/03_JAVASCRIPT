...
btn2.addEventListener("click",()=>{
  ...
  /* 
    ...
    2) <buttn onclick="testFn(this)">테스트</button>
      여기서 this == 이벤트가 발생한 요소 (클릭된 button)

    3) 화살표 함수 내에서 this == Window 객체
      (화살표 함수에서는 this 못씀 생각도하지마라!!!)
  */
...
function Student(inputGrade, inputBan, inputNumber, inputName){
  ...
  this.information = function(){
    let str = 
    `${this.grade}학년 ${this.ban} 반 ${this.number} ${this.name}`;
...
addStudent.addEventListener("click",()=>{
  ...
  // 이전 입력값 삭제
  inputGrade.value= ''; // 빈 문자열 value에 대입해 삭제한 효과
  inputBan.value= ''; // 빈 문자열 value에 대입해 삭제한 효과
  inputNumber.value= ''; // 빈 문자열 value에 대입해 삭제한 효과
  inputName.value= ''; // 빈 문자열 value에 대입해 삭제한 효과
})
...
checkJSON.addEventListener("click", () => {
  ...
  // JSON 생성 (JS객체 모양의 문자열)
  const json2 = '{"menu":"김밥", "price":4000}';
  
  console.log("json2",json2,typeof json2);
  ...
  // 기상청 서버에 7/12 현재 전국 미세먼지 농도를 JSON으로 받아오기
